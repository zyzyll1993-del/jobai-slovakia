/* JobAI Slovakia — Resume editor builder layout + multilingual navigation */
(function () {
  "use strict";
  function getLabels(){
  ...
  function init(){
    ...
    var shell=document.createElement("div"), side=document.createElement("aside"), main=document.createElement("div");
    shell.className="jobai-editor-shell"; side.className="jobai-editor-sidebar"; main.className="jobai-editor-main";
    var l=getLabels(); side.innerHTML='<div class="jobai-editor-brand">'+l.title+'</div><div class="jobai-editor-save">● '+l.done+'</div><div class="jobai-editor-nav"></div>';
    ...
    cards.forEach(function(card,i){
      var heading=card.querySelector("h2,h3"), title=heading?heading.textContent.trim():l.sections[i]||("Section "+(i+1));
      if(i<l.sections.length) title=l.sections[i];
      card.id=card.id||("jobai-editor-section-"+i); card.classList.add("jobai-editor-card");
      var b=document.createElement("button"); b.type="button"; b.className="jobai-editor-nav-item"; b.innerHTML='<span class="jobai-editor-number">'+(i+1)+'</span><span class="jobai-nav-text">'+title+'</span>';
