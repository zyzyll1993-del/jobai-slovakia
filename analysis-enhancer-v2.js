...
function loadScript(id,src,onload){var existing=document.getElementById(id);if(existing){if(onload&&existing.dataset.loaded==='1')onload();return existing;}var s=document.createElement('script');s.id=id;s.src=src;s.onload=function(){s.dataset.loaded='1';if(onload)onload();};s.onerror=function(){s.dataset.loaded='0';};document.head.appendChild(s);return s;}
function ensureRecommendations(){if(typeof window.renderJobRecommendations==='function'){setTimeout(window.renderJobRecommendations,20);return;}loadScript('jobaiRecommendationsScript','job-recommendations.js?v=5',function(){if(typeof window.renderJobRecommendations==='function')setTimeout(window.renderJobRecommendations,20);});}
function openJobs(){if(document.getElementById('jobaiJobsPageScript')){if(typeof window.showTab==='function')window.showTab('jobs');return;}loadScript('jobaiJobsPageScript','jobs-page.js?v=6',function(){setTimeout(function(){if(typeof window.showTab==='function')window.showTab('jobs');},20);});}
function installLazyJobs(){var nav=document.querySelector('.nav');if(!nav)return;var b=document.getElementById('navJobs');if(!b){b=document.createElement('button');b.id='navJobs';var analysis=document.getElementById('navAnalysis');if(analysis)nav.insertBefore(b,analysis);else nav.appendChild(b);}b.textContent=tr().jobs;b.onclick=openJobs;}
var oldSetLanguage=window.setLanguage;if(typeof oldSetLanguage==='function'){window.setLanguage=function(l){oldSetLanguage(l);setTimeout(function(){var b=document.getElementById('navJobs');if(b)b.textContent=tr().jobs;},20);};}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',installLazyJobs);else installLazyJobs();
})();