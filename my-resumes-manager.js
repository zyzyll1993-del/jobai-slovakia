...
(function () {
  ...
  function render(){var box=document.getElementById('jobaiMyResumesV3');if(!box)return;var a=ensure(),active=activeId(),html='<div style="font-size:21px;font-weight:800;margin-bottom:12px">📁 '+txt('title')+'</div>';
    ...
    a.forEach(function(x){var flag=FLAGS[x.language]||'🌐';active=x.id===active;html+='<div data-jr-card="'+x.id+'" style="'+cardStyle()+'"><div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start;flex-wrap:wrap"><div><div style="font-weight:800;font-size:16px">'+(active?'● ':'')+flag+' '+escapeHtml(displayName(x.name))+(x.primary?' ⭐':'')+'</div><div style="font-size:12px;opacity:.7;margin-top:3px">'+escapeHtml(languageName(x.language))+' · '+txt('changed')+fmt(x.updatedAt)+'</div></div><div style="display:flex;flex-wrap:wrap;justify-content:flex-end">'+button(txt('open'),'data-open="'+x.id+'"')+button('📋 '+txt('dup'),'data-dup="'+x.id+'"')+button('✏️ '+txt('ren'),'data-ren="'+x.id+'"')+button('⭐ '+txt('primary'),'data-primary="'+x.id+'"')+button('📄 '+txt('pdf'),'data-pdf="'+x.id+'"')+button('🗑️ '+txt('del'),'data-del="'+x.id+'"')+'</div></div></div>';});
