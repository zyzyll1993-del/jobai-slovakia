...
function render(details){var result=document.getElementById('analysisResult');if(!result)return;result.style.display='block';var u=tr(),scoreEl=document.getElementById('score'),bar=document.getElementById('progressBar');if(scoreEl)scoreEl.textContent=details.score+'%';if(bar)bar.style.width=details.score+'%';
 var mm=matchedMissing(details),found=document.getElementById('found'),missing=document.getElementById('missing'),rec=document.getElementById('recommendation');if(found)found.innerHTML=list(mm.good);if(missing)missing.innerHTML=list(mm.bad);if(rec)rec.textContent=details.score>=70?u.good:details.score>=40?u.partial:u.low;
 ...
 var rows=details.parts.map(function(p){return '<div style="display:grid;grid-template-columns:minmax(120px,.8fr) 80px minmax(180px,2fr);gap:10px;align-items:start;padding:10px 0;border-bottom:1px solid rgba(127,127,127,.18)"><strong>'+esc(p.label)+'</strong><span style="font-weight:800">'+p.score+'%</span><span>'+esc(statusText(p))+'</span></div>';}).join('');
