(function(){
  ...
  function setup(type){
    ...
    function render(){
      ...
      list.querySelectorAll('[data-f]').forEach(function(el){el.addEventListener('input',function(){var a=getArr();if(a[Number(el.dataset.i)])a[Number(el.dataset.i)][el.dataset.f]=el.value;save();update();});});
      list.querySelectorAll('[data-remove]').forEach(function(btn){btn.addEventListener('click',function(){var a=getArr();a.splice(Number(btn.dataset.remove),1);render();save();update();if(!a.length){list.style.display='none';toggle.setAttribute('aria-expanded','false');toggle.innerHTML='➕ '+(isExp?'Досвід роботи':'Освіта');}});});
    }
    function add(){
